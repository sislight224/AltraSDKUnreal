...
void UAlturaSDKBPLibrary::GetCollections(const FString perPage, const FString page, const FString ownerAddress, SortBy SortBy, SortDir Sortdir, const FResponse& OnComplete)
{

	TMap<FString, FString> Params;
	Params.Add("perPage", perPage);
	Params.Add("page", page);
	Params.Add("ownerAddress", ownerAddress);
	Params.Add("sortBy", SortbyEnumToString(SortBy));
	Params.Add("sortDir", SortDirEnumToString(Sortdir));
...
TArray<FCollection> UAlturaSDKBPLibrary::ParseGetCollections(const FString& ResponseString)
{
	...
	for (const FAlturaJSONObject& CollectionObject : CollectionsArray)
	{
		...
		TempCollection.ImageUrl = GetStringFromJSONObject(CollectionObject, "ImageUrl");
